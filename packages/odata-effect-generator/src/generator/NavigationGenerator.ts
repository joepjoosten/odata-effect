...
const generatePathBuildersFile = (dataModel: DataModel): string => {
  ...
  // Terminal operations (Promise-based)
  lines.push(`// ============================================================================`)
  lines.push(`// Terminal Operations (Promise-based)`)
  lines.push(`// ============================================================================`)
  lines.push(``)
  lines.push(`/**`)
  lines.push(` * Fetch a collection of entities at the given path (Promise-based).`)
  lines.push(` *`)
  lines.push(` * @example`)
  lines.push(` * \`\`\`typescript`)
  lines.push(` * const path = pipe(People, byKey("russell"), trips)`)
  lines.push(` * const myTrips = await fetchCollectionPromise(runtime, path, Trip)`)
  lines.push(` * \`\`\``)
  lines.push(` *`)
  lines.push(` * @since 1.0.0`)
  lines.push(` * @category operations`)
  lines.push(` */`)
  lines.push(`export const fetchCollectionPromise = <T, I>(`)
  lines.push(`  runtime: ${runtimeType},`)
  lines.push(`  path: Path<T, true>,`)
  lines.push(`  schema: Schema.Schema<T, I>,`)
  lines.push(`  options?: ${versionConfig.queryOptionsType}`)
  lines.push(`): Promise<ReadonlyArray<T>> =>`)
  lines.push(`  runtime.runPromise(${versionConfig.odataNamespace}.getCollection(path, schema, options))`)
  lines.push(``)
  lines.push(`/**`)
  lines.push(` * Fetch a single entity at the given path (Promise-based).`)
  lines.push(` *`)
  lines.push(` * @example`)
  lines.push(` * \`\`\`typescript`)
  lines.push(` * const path = pipe(People, byKey("russell"))`)
  lines.push(` * const person = await fetchOnePromise(runtime, path, Person)`)
  lines.push(` * \`\`\``)
  lines.push(` *`)
  lines.push(` * @since 1.0.0`)
  lines.push(` * @category operations`)
  lines.push(` */`)
  lines.push(`export const fetchOnePromise = <T, I>(`)
  lines.push(`  runtime: ${runtimeType},`)
  lines.push(`  path: Path<T, false>,`)
  lines.push(`  schema: Schema.Schema<T, I>,`)
  lines.push(`  options?: ${versionConfig.queryOptionsType}`)
  lines.push(`): Promise<T> =>`)
  lines.push(`  runtime.runPromise(${versionConfig.odataNamespace}.get(path, schema, options))`)
  lines.push(``)

...
for (const sample of ["odata-v2", "trippin"]) {
  for (const granular of [false, true]) {
    // Allow cold compiler startup under CI load without changing ordinary unit-test timeouts.
    it.effect(
      `compiles generated ${sample} ${
        granular ? "granular" : "legacy"
      } models and services against the installed runtime`,
      () =>
        Effect.gen(function*() {

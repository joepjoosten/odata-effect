...
import { ODataError, ParseError, type SapError } from "./Errors.js"
...
export const ODataCollectionResponse = <A, I, R>(schema: Schema.Codec<A, I, R>) =>
  Schema.Union([
    ...
    Schema.Struct({
      ...
      "odata.metadata": Schema.optional(Schema.NullishOr(Schema.String))
    })
  ])
...
export const ODataCollectionResponseWithMeta = <A, I, R>(schema: Schema.Codec<A, I, R>) =>
  Schema.Union([
    ...
    Schema.Struct({
      d: Schema.Struct({
        results: Schema.Array(schema),
        __count: Schema.optional(Schema.NullishOr(Schema.String)),
        __next: Schema.optional(Schema.NullishOr(Schema.String))
      })
    }),
    // V2 Legacy: { d: [...] }
    Schema.Struct({
    ...
    Schema.Struct({
      value: Schema.Array(schema),
      "odata.metadata": Schema.optional(Schema.NullishOr(Schema.String)),
      "odata.count": Schema.optional(Schema.NullishOr(Schema.Union([Schema.String, Schema.Number]))),
...
export const EntityMetadata = Schema.Struct({
  uri: Schema.String,
  type: Schema.optional(Schema.NullishOr(Schema.String)),
  etag: Schema.optional(Schema.NullishOr(Schema.String)),
...
export const MediaMetadata = Schema.Struct({
  ...
  media_etag: Schema.optional(Schema.NullishOr(Schema.String)),
  edit_media: Schema.optional(Schema.NullishOr(Schema.String)),
  content_type: Schema.optional(Schema.NullishOr(Schema.String))
})
  ...
  HttpClientResponse.schemaBodyJson(schema)(response).pipe(
    Effect.mapError((error) =>
      Schema.isSchemaError(error)
        ? new ParseError({
          message: "Failed to decode OData response body",
          cause: error
        })
        : error)
  )
...
> => {
  ...
  return Effect.gen(function*() {
    ...
    const response = yield* client.execute(request)
    const data = yield* decodeJsonBody(wrappedResponseSchema)(response)
    // Handle V2 ({ d: Entity }) and V3/V4 (Entity at root) formats
    if (data !== null && typeof data === "object" && "d" in data) {
      return (data as { readonly d: A }).d
    }
